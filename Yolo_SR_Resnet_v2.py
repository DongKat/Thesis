...
# dataset = NomDataset_Yolo(LVT_RAW, annotations, LVT_RAW_ANNOTATIONS)
...
import PIL as pil
crop_path = 'TempResources/newt'
dest = 'TempResources/lrbix2'
for idx, crop in tqdm(enumerate(os.listdir(crop_path), start=0)):
    # use pillow to open the image
    crop_i = pil.Image.open(os.path.join(crop_path, crop))
    # resize the image to smaller 2x
    crop_i = crop_i.resize((128, 128), pil.Image.BICUBIC)
    # save the image
    crop_i.save(os.path.join(dest, crop))
