...
class NomDatasetCrop(Dataset):
        ...
        def read_label(label_path):
            with open(label_path, 'r') as f:
                for line in f.readlines():
                    line = line.split(' ')
                    self.labels_list.append(line[1])
    ...
    def __getitem__(self, idx):
        ...
        if (h, w) != self.input_size:
            x_crop_img = cv2.resize(x_crop_img, self.input_size, cv2.INTER_CUBIC)
        ...
        x_crop_img = preprocess(x_crop_img)
...
opt_dict = {
    'train': ('TempResources/ToK1871/ToK1871_crops', 'TempResources/ToK1871/ToK1871_crops.txt'),
...
datamodule = NomDataModule(opt_dict, 8, (64, 64), 0)
